...
describe('configWarnings', () => {
  ...
  it('flags missing PayPal credentials as one warning', () => {
    ...
    expect(configWarnings(env, cf).filter((m) => m.text.includes('PayPal is enabled but'))).toHaveLength(1);
  });
  ...
  it('flags cloudflare email provider without the send_email binding', () => {
    ...
    expect(configWarnings(env, cf).some((m) => m.text.includes('send_email binding'))).toBe(true);
  ...
  it('nudges toward Access when running on password auth', () => {
    ...
    expect(configWarnings(env, cf).some((m) => m.text.includes('password-based'))).toBe(true);
  ...
  it('flags a missing From address', () => {
    ...
    expect(configWarnings(fullEnv, noFrom).some((m) => m.text.includes('From address'))).toBe(true);
  });
  ...
  it('only requires a Resend key when Resend is selected', () => {
    ...
    expect(configWarnings(env, resend).some((m) => m.text.includes('Resend API key'))).toBe(true);
  });
  ...
  it('suppresses only the PayPal webhook-id warning in local dev', () => {
    ...
    expect(configWarnings(env, cf).some((m) => m.text.includes('PAYPAL_WEBHOOK_ID'))).toBe(true);
    ...
    expect(configWarnings(noStripe, cf, { localDev: true }).some((m) => m.text.includes('STRIPE_SECRET_KEY'))).toBe(true);
  });
  ...
  it('one enabled provider is enough to avoid the no-payments notice', () => {
    ...
    expect(configWarnings(fullEnv, on).some((m) => m.text.includes('No payment methods'))).toBe(false);
  });
